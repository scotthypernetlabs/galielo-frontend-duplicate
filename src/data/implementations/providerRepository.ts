...
export class ProviderRepository implements IProviderRepository {
  ...
  public stakeTokensRequest(amount: number) {
    ...
    this.socket.emit("stake_tokens_request", amount).then((response: any) => {
      logService.log("Stake tokens request", response);
    });
  }
  public stakeTokensStatus(
    stake_id: string,
    status: string,
    amount: number,
    receiptId: string
    ...
    this.socket
      .emit("stake_tokens_status", stake_id, status, amount, receiptId)
      .then((response: any) => {
        logService.log("Stake tokens status", response);
      });
  }
  public stakeTokensSuccess(stake_id: string, receiptId: string) {
    this.socket.emit("stake_tokens_success", stake_id, receiptId).then(() => {
      logService.log("Emitted stake tokens success");
      store.dispatch(
        openNotificationModal("Notifications", "Staked tokens successfully")
      );
    });
  }
  public stakeTokensFailure(stake_id: string) {
    this.socket.emit("stake_tokens_failure", stake_id).then(() => {
  ...
  public createOfferRequest(
    ...
    logService.log(
      `Making offer with rate=${rate} pay_interval=${pay_interval} max_acceptances=${max_acceptances} deposit_per_acceptance=${deposit_per_acceptance} expiration_date=${expiration_date}`,
      mids
    );
    this.socket
      .emit(
  ...
  public getWallet(offer_id: string, total_stake_balance: number) {
    this.socket.emit("wallet", offer_id, total_stake_balance).then(() => {
  ...
  public paymentReceived(receiptId: string) {
    this.socket.emit("payment_received", receiptId).then(() => {
      logService.log("Payment received");
    });
  }
  public deleteOffer(offerId: string) {
    this.socket.emit("dekete", offerId).then(() => {
      this.offerService.updateOffers();
