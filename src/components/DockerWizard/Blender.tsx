...
class BlenderWizard extends React.Component<Props, State> {
  ...
  componentDidMount() {
    ...
    const fileString =
      frameworkExplanation + `FROM hyperdyne/blender:2.79\n\nCOPY . /media\n\n`;
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    if (prevState !== this.state) {
      if (this.state.outputTo.length > 0 && this.state.selectedFormat) {
        ...
        const entrypoint2 =
          this.state.mode === "--render-frame"
            ? `"${this.state.mode}", "${this.generateFrameRange(
                this.state.frameRange
              )}"]`
            : `"${this.state.mode}", "${this.state.frameRange}", "-a"]`;
    ...
    return (
      <>
        <div className="blender-grid">
          ...
          />
          <button
            onClick={this.toggleFrameRange}
            className="frame-range-toggle"
          >
            {this.state.mode === "--render-frame" ? "Frame Range" : "Scene"}
          </button>
          <input
            placeholder={
              this.state.mode === "--render-frame"
                ? `e.g. 1..7 or 1,2,5 or 1-7`
                : `e.g Scene1`
            }
