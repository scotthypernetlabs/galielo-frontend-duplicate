...
class HecrasWizard extends React.Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Props, prevState: State){
    let explanation;
    let frameworkText =  '#The line below determines the build image to use\n\n' + `FROM hyperdyne/simulator:hecras\n\n`;
    ...
    if(change){
      ...
      let entrypoint = 'HECRAS';
      this.props.receiveDockerInput({
        dockerTextFile: finalizedDockerFile,
        entrypoint: entrypoint
      })
    }
    if(this.props.state.entrypoint !== "set" && !this.state.networkFileSystem){
      this.props.receiveDockerInput({
        entrypoint: 'set'
      })
    }
