...
describe("JobsButtonGroup Component", () => {
  ...
  it("buttons should have the right text", () => {
    ...
    expect(jobsButtonGroup.find(ToggleButton).at(0).childAt(0).text()).toBe("Sent");
    expect(jobsButtonGroup.find(ToggleButton).at(1).childAt(0).text()).toBe("Received");
  });

  it("buttons should call toggleMode", () => {
    jobsButtonGroup.find(ToggleButton).at(0).simulate("click");
    expect(jobsButtonGroup.childAt(0).props().toggleMode).toHaveBeenCalled();
    jobsButtonGroup.find(ToggleButton).at(1).simulate("click");
    expect(jobsButtonGroup.childAt(0).props().toggleMode).toHaveBeenCalled();
  });
